...
export default function Testimonial({
  ...
  company,
  rating = 5,
  avatarSrc = "/assets/Gabriele-Mirabile.jpg",
  className = "",
}: TestimonialProps) {
  return (
    <div className={`relative w-[85%] md:w-[80%] ml-auto ${className}`}>
      ...
      <div className="bg-transparent  rounded-lg p-2  md:p-2 relative pb-2 md:pb-8">
        ...
        <div className="flex flex-row gap-2 md:gap-6 items-start">
          ...
          <div className="flex-1 text-right">
            ...
            {/* <div className="flex gap-0.5 md:gap-1 justify-end mb-0.5 md:mb-2">
              {Array.from({ length: 5 }).map((_, index) => (
                <svg
                  key={index}
                  className={`w-3 h-3 md:w-5 md:h-5 ${index < rating ? "text-yellow-400" : "text-gray-300"
                    }`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
            ...
            {/* Quote text */}
            <TezzeractText variant="dark" className="block mb-0.5 md:mb-2 text-[1px] md:text-base leading-tight pl-8">
              &ldquo;{quote}&rdquo;
            </TezzeractText>
          ...
          <div className="shrink-0">
            <div className="relative w-12 h-12 md:w-18 md:h-18 rounded-xl overflow-hidden bg-gray-200">
              <Image
                src={avatarSrc}
                alt={name}
                fill
